  ...
  return (
    ...
    >
      ...
      <div className="flex h-[72px] shrink-0 items-center border-b border-[var(--sidebar-border)] px-5">
        ...
        >
          {/* Sur fond sombre, l'accent bleu manquerait de contraste : la marque
              passe en blanc, le jaune restant lisible. Police du logotype :
              voir lib/fonts.ts, seule police téléchargée de l'application. */}
          <span
            className={cn(wordmarkFont.className, "text-[30px] leading-none text-[var(--sidebar-text)]")}
            style={{ fontWeight: 800, fontVariationSettings: "'opsz' 72", letterSpacing: "0.5px" }}
          >
            MG
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-3" aria-label={t("nav.main")}>
        {NAV.map((group, index) => (
          <div key={group.labelKey ?? `group-${index}`} className={index > 0 ? "mt-5" : undefined}>
            {group.labelKey && (
              <h2
                className="px-3 pb-1.5 text-[12px] font-semibold uppercase tracking-wide"
                    ...
                    <li key={item.href}>
                      <span
                        ...
                        className={cn(
                          ...
                          "text-[15px] font-medium text-[var(--sidebar-text-muted)] opacity-50",
                        )}
                      >
      ...
      <div className="shrink-0 border-t border-[var(--sidebar-border)] px-5 py-3">
        <p className="text-[13px] text-[var(--sidebar-text-muted)]">{t("app.owner")}</p>
      </div>
    </aside>
  );
}
